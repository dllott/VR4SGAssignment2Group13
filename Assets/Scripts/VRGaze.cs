...
/*  This is for later on when we are adding the events
        ...
        {
            if (imgGaze.fillAmount == 0 && _hit.transform.CompareTag("door"))
            {
                _hit.transform.gameObject.GetComponent<OpenDoor>().goUp();
            }

            if (imgGaze.fillAmount == 0 && _hit.transform.CompareTag("teleportBad"))
            {
                _hit.transform.gameObject.GetComponent<Teleport>().TeleportPlayerBad();
            }
